 ...
 * Travis County's ArcGIS instance flakes ~50% of calls, so a single retry
 * still leaves ~1 in 4 requests failing -- needs the extra attempt.
 */
async function retry(fn, attemptsLeft = 2) {
  try {
    ...
    // Travis County's ArcGIS instance intermittently 400s "Failed to execute
    // query" on otherwise-valid requests (confirmed: same params pass/fail
    // on alternating calls) -- treat as transient like 5xx.
